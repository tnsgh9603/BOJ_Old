...
#endif
vector <InfInt> vec; // 숫자만 담는 vec
vector <char> last; // 연산자만 담는 last
int main() {
    fastio;
	// 필요한 변수 선언
	InfInt N, input_num, a;
	char input_char;
	cin >> N;
	
	// 숫자가 연산자보다 하나 더 입력받으므로 for문 앞에서 미리 숫자 하나 받음
	cin >> input_num;
	// 받고 vec에 넣음
	...
	for (InfInt i = 1; i <= N - 1; i++) {
		...
		// 받은 연산자가 '*'나 '/' 이면 바로 계산
		if (input_char == '*') {
			vec.back() *= input_num; // 벡터 맨 뒤에 있는 숫자를 꺼내와 바로 연산함
		}
		// 나눗셈을 해야할 때
		if (input_char == '/') {
			// 둘 다 음 수 일때, 둘 다 정수로 바꿔서 계산함
			if (input_num < 0 && vec.back() < 0) {
				input_num *= -1;
				vec.back() *= -1;
			}
			// 둘 중 하나만 음수면
			if (input_num < 0 || vec.back() < 0) {
				// 둘 중 하나가 음수여도 딱 나눠 떨어지면 그대로 계산
				if (vec.back() % input_num == 0) {
					vec.back() = vec.back() / input_num;
				}
				// 
				else {
					vec.back() = (vec.back() / input_num) - 1;
				}
			}
			else {
				vec.back() = (vec.back() / input_num);
